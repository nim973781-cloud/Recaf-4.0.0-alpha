...
public class MappingApplicationPane extends BorderPane {
	...
	private HBox createButtonBar() {
		...
		disableDirImport.bind(formatBox.valueProperty().map(f -> f == null || !f.supportsDirectoryLoading()));
		...
		Button setMappingDir = new ActionButton(CarbonIcons.FOLDER, Lang.getBinding("mapapply.pick.dir"), () -> {
			MappingFileFormat format = formatBox.getValue();
			if (format == null || !format.supportsDirectoryLoading()) {
			...
			if (file != null) {
				try {
					IntermediateMappings mappings = format.parse(file.toPath());
					logger.info("Loaded {} directory mappings from {}", format.implementationName(), file.getName());
					mappingsProperty.set(mappings);
				} catch (Throwable t) {
					logger.error("Failed importing mappings from {}", file.getName(), t);
				}
