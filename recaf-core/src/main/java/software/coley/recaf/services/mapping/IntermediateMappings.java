...
public class IntermediateMappings implements Mappings {
	...
	public Set<String> getClassesWithMappings() {
		...
		variables.values().forEach(variableMappings -> variableMappings.stream()
				.map(VariableMapping::getOwnerName)
				.forEach(set::add));
		return set;
	}
	...
	/**
	 * @return Global method mappings (owner-independent), used for formats like SRG where
	 *         method names (m_12345_) are globally unique. Default implementation returns empty map.
	 */
	@Nonnull
	...
	/**
	 * @return Global field mappings (owner-independent), used for formats like SRG where
	 *         field names (f_12345_) are globally unique. Default implementation returns empty map.
	 */
	@Nonnull
