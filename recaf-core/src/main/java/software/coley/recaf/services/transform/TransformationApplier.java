...
public class TransformationApplier {
	...
	public JvmTransformResult transformJvm(@Nonnull List<Class<? extends JvmClassTransformer>> transformerClasses,
		...
		// Get config for error handling
		TransformationManagerConfig config = transformationManager.getServiceConfig();
		boolean continueOnTransformerFailure = config.getContinueOnTransformerFailure().getValue();
		...
		// Track if we should abort due to transformer failure (when continueOnTransformerFailure is false)
		...
		resource.jvmClassBundleStreamRecursive().forEach(bundle -> {
			...
			for (int pass = 1; pass <= getMaxPasses(); pass++) {
				...
				for (JvmClassTransformer transformer : transformers) {
					// Check if we should abort due to previous failure
					if (shouldAbort.get())
						return;
					...
					bundle.forEach(cls -> {
						...
						try {
							...
							// If we should not continue on transformer failure, mark for abort
							if (!continueOnTransformerFailure) {
				...
				// Break if no work has been done this pass or if we should abort
				if (!anyWorkDone.get() || shouldAbort.get())
					break;
			}
		});

		// If we should abort due to transformer failure, throw exception with all failures
		if (shouldAbort.get() && !transformJvmFailures.isEmpty()) {
			var firstFailure = transformJvmFailures.values().iterator().next().values().iterator().next();
			throw new TransformationException("Transformer failed and continueOnTransformerFailure is disabled", firstFailure);
		}

		// Update the workspace contents with the transformation results
		boolean continueOnSerializationFailure = config.getContinueOnSerializationFailure().getValue();
		Map<ClassPathNode, JvmClassInfo> transformedJvmClasses = context.buildChangeMap(inheritanceGraph, continueOnSerializationFailure);
