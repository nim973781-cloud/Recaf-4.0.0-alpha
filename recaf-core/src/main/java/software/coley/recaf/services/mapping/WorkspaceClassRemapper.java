...
public class WorkspaceClassRemapper extends ClassRemapper {
	...
	public MethodVisitor visitMethod(int access, @Nonnull String name, @Nonnull String descriptor,
		...
		return mv == null ? null : new VariableRenamingMethodVisitor(access, className, name, descriptor, mv, workspaceRemapper);
	}
	...
	private class VariableRenamingMethodVisitor extends MethodRemapper {
		...
		private final int[] parameterSlotIndices;
		private int parameterOrder;

		public VariableRenamingMethodVisitor(int access,
		                                     @Nonnull String methodOwner, @Nonnull String methodName, @Nonnull String methodDesc,
		                                     @Nullable MethodVisitor mv, @Nonnull Remapper remapper) {
			super(RecafConstants.getAsmVersion(), mv, remapper);
			this.methodOwner = methodOwner;
			this.methodName = methodName;
			this.methodDesc = methodDesc;
			this.parameterTypes = Type.getArgumentTypes(methodDesc);
			this.parameterSlotIndices = new int[parameterTypes.length];

			int slot = (access & org.objectweb.asm.Opcodes.ACC_STATIC) != 0 ? 0 : 1;
			for (int i = 0; i < parameterTypes.length; i++) {
				parameterSlotIndices[i] = slot;
		...
		public void visitParameter(String name, int access) {
			...
			if (parameterOrder < parameterTypes.length) {
				Type parameterType = parameterTypes[parameterOrder];
				int slotIndex = parameterSlotIndices[parameterOrder];
				mappedName = workspaceRemapper.mapVariableName(
						methodOwner,
						methodName,
						methodDesc,
						name,
